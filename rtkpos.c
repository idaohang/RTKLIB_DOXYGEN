...
/* baseline length constraint 基线长度限制------------------------------------------------*/
...
/**
 * @brief relative positioning 相对定位
 * @details 貌似是差分
 * 
 * @param rtk rtk结构体指针
 * @param obs observation data record 观测数据结构体指针
 * @param nu [description]
 * @param nr [description]
 * @param nav navigation data type 导航数据类型指针
 * @return state为SOLQ_NONE是，返回0，否则返回1
 */
                  ...
                  const nav_t *nav)
{
    ...
    /* satellite positions/clocks 计算卫星位置和时钟*/
    satposs(time,obs,n,nav,opt->sateph,rs,dts,var,svh);
    
    /* undifferenced residuals for base station 基准站残差*/
    ...
    /* time-interpolation of residuals (for post-processing) 残差时间插值*/
    if (opt->intpref) {
    ...
    for (i=0;i<niter;i++) {
        ...
        /* double-differenced residuals and partial derivatives 双差和偏导残差*/
    ...
    if (stat!=SOLQ_NONE&&zdres(0,obs,nu,rs,dts,svh,nav,xp,opt,0,y,e,azel)) {
        ...
        if (valpos(rtk,v,R,vflg,nv,4.0)) {
            ...
            /* update ambiguity control struct 模糊度控制*/
            rtk->sol.ns=0;
