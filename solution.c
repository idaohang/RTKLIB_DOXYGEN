...
/**
 * @brief 解算质量标示数组
 * @details nmea 0183 v.2.3 quality flags:\n
 *  0=invalid, 1=gps fix (sps), 2=dgps fix, 3=pps fix, 4=rtk, 5=float rtk6=estimated (dead reckoning), 7=manual input, 8=simulation
 */
...
/**
 * @brief solution option to field separator
 * @details 把解算参数转换成字段分隔符器
 * 
 * @param opt 解算参数结构体指针
...
/**
 * @brief separate fields 分隔字段
 * @details 把缓冲里的字符串分隔，并转成浮点数值
 * 
 * @param buff 缓冲向量
 * @param sep 分隔符
 * @param v double数组向量
...
/**
 * @brief convert ddmm.mm in nmea format to deg
 * @details 将NMEA里的dmm浮点值转换成角度值
 * 
 * @param dmm dmm浮点数
 * @return 角度浮点数
 */
...
/**
 * @brief convert time in nmea format to time
 * @details 把NMEA格式时间转换成时间
 * 
 * @param t nmea格式时间
 * @param t1 时
 * @param t2 分
